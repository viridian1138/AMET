...
/**
 * Font plug-in for the Kenney Future font
 * 
 * @author tgreen
 *
 */
public class FontKenneyFuture implements IFont {
	...
	public boolean isFontLoaded(SessionDataApplicationToken sess) {
		...
		synchronized (FontResponseHandler.syncObj) {
			SessionFontLoadingDesc desc = (SessionFontLoadingDesc) (std.objects
					.get(FontResponseHandler.KENNEY_FUTURE_FONT));
			if (desc == null) {
				desc = new SessionFontLoadingDesc();
				std.objects.put(FontResponseHandler.KENNEY_FUTURE_FONT, desc);
			}
