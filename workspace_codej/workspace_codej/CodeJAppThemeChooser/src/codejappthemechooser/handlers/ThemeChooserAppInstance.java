...
package codejappthemechooser.handlers;
...
/**
 * Class for instances of a theme chooser Metaverse Application
 * 
 * @author tgreen
 *
 */
public class ThemeChooserAppInstance extends MetaverseApplicationInstance {
	...
	/**
	 * Widget for displaying the current theme
	 */
	protected TextWidget themeTextWidget = null;
	...
	/**
	 * Vertical scroll bar for scrolling through the themes
	 */
	protected AbstractScrollbarWidget verticalScrollbarWidget = null;

	/**
	 * The current index into the theme list
	 */
	protected int currentIndex = 0;

	/**
	 * The sorted list of themes
	 */
	protected ArrayList<IMetaverseTheme> sortedThemes = null;
	 ...
	 * Default Constructor
	 */
	public ThemeChooserAppInstance() {
	}
	...
	public void dispatchEvent(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
		...
		gc.appendJs("{\n");
		gc.appendJs("tmpObj.zmsg=\"This Is A Theme Chooser Response\";\n");
		gc.appendJs("}\n");
	}
	...
	public void dispatchStartEvent(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
		...
		sortedThemes = new ArrayList<IMetaverseTheme>();

		TreeMap<String, IMetaverseTheme> tm = new TreeMap<String, IMetaverseTheme>();
		...
		for (IMetaverseTheme thm : SessionStore.themes.values()) {
			tm.put(thm.getThemeName(), thm);
		}

		for (IMetaverseTheme thm : tm.values()) {
			sortedThemes.add(thm);
		}

		if ((currentIndex >= 0) && (currentIndex < sortedThemes.size())) {
			System.out.println("Starting With Theme " + (sortedThemes.get(currentIndex).getThemeName()));

			final String themeName = sortedThemes.get(currentIndex).getThemeName();

			{
				TextWidgetInitializer twA = new TextWidgetInitializer(theme);
				twA.setDisplayText("Running Theme Chooser");
...

			{
				...
				twB.setDisplayText(themeName);
				twB.setPosition(new Vector3(0.0, 3.5, -7.0));

				themeTextWidget = TextWidget.create(gc, twB);
				themeTextWidget.addToScene(gc);
			}
...

		{
			...
			focus.add("Apply Button", applyButton, new IEventHandler() {
				...
				public void dispatchEvent(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
					...
					System.out.println("Theme Chooser Attempting Theme Apply");

					stopApplication(e, sess, gc);

					IMetaverseTheme itheme = sortedThemes.get(currentIndex);

					System.out.println("Changing To Theme " + (itheme.getThemeName()));

					gc.changeTheme(itheme, e);
...

		{
			...
			pwi.setMinScroll(0);
			pwi.setMaxScroll(sortedThemes.size());
			...
			focus.add("Vertical Scrollbar", verticalScrollbarWidget, new IEventHandler() {
				...
				public void dispatchEvent(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
					...
					if (keyCode.equals(KeyNames.ARROW_DOWN_KEY)) {
						System.out.println("Theme Chooser Attempting Theme Increment");

						currentIndex++;

						if (currentIndex >= sortedThemes.size()) {
							currentIndex = 0;
						}

						if ((currentIndex >= 0) && (currentIndex < sortedThemes.size())) {
							System.out
									.println("Incremented To Theme " + (sortedThemes.get(currentIndex).getThemeName()));

							final String themeName = sortedThemes.get(currentIndex).getThemeName();

							final int arrivedCurrentIndex = currentIndex;

							themeTextWidget.setDisplayText(themeName, gc);

							{
								verticalScrollbarWidget.setLocation(0, sortedThemes.size(), arrivedCurrentIndex,
					...
					if (keyCode.equals(KeyNames.ARROW_UP_KEY)) {
						System.out.println("Theme Chooser Attempting Theme Decrement");

						currentIndex--;

						if (currentIndex < 0) {
							currentIndex = sortedThemes.size() - 1;
						}

						if ((currentIndex >= 0) && (currentIndex < sortedThemes.size())) {
							System.out
									.println("Decremented To Theme " + (sortedThemes.get(currentIndex).getThemeName()));
	...
	public void dispatchKeydownEvent(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
		...
		System.out.println("Theme Chooser App Responding To Keydown Event");
	...
	public void stopApplication(UiEvent e, SessionDataApplicationToken sess, GraphicsContext gc) {
		...
		{
			themeTextWidget.removeFromScene(gc);

			themeTextWidget.dispose(gc);
		}
		...
		gc.appendJs("{\n");
		gc.appendJs("tmpObj.zmsg=\"This Is A Theme Chooser Stop Resonse\";\n");
		gc.appendJs("}\n");
