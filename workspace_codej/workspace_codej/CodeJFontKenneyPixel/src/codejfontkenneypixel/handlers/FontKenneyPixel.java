...
/**
 * Font plug-in for the Kenney Pixel font
 * 
 * @author tgreen
 *
 */
public class FontKenneyPixel implements IFont {
	...
	public boolean isFontLoaded(SessionDataApplicationToken sess) {
		...
		synchronized (FontResponseHandler.syncObj) {
			SessionFontLoadingDesc desc = (SessionFontLoadingDesc) (std.objects
					.get(FontResponseHandler.KENNEY_PIXEL_FONT));
			if (desc == null) {
				desc = new SessionFontLoadingDesc();
				std.objects.put(FontResponseHandler.KENNEY_PIXEL_FONT, desc);
			}
