...
/**
 * Font plug-in for the Commodore font
 * 
 * @author tgreen
 *
 */
public class FontJseCommodore implements IFont {
	...
	public boolean isFontLoaded(SessionDataApplicationToken sess) {
		...
		synchronized (FontResponseHandler.syncObj) {
			SessionFontLoadingDesc desc = (SessionFontLoadingDesc) (std.objects
					.get(FontResponseHandler.JSE_COMMODORE_FONT));
			if (desc == null) {
				desc = new SessionFontLoadingDesc();
				std.objects.put(FontResponseHandler.JSE_COMMODORE_FONT, desc);
			}
